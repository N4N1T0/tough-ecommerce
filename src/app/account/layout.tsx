...
export default async function AccountLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <main className='max-w-[1400px] px-5 mx-auto grid grid-cols-5 grid-rows-1 pt-48 gap-10'>
      <aside className='h-full col-span-1 row-span-1'>
        <ul className='px-5'>
          {accountLinks.map((item, idx) => (
            <li key={`Account-link-${item.label}-${idx}`} className='uppercase text-md font-semibold w-full pt-5'>
              <Link className='hover:underline mt-3' href={item.link}>{item.label}</Link>
              <hr className='h-[1px] border-gray-500 mt-3' />
