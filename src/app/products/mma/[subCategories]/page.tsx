...
export default async function MMAPage({ params }: { params: { subCategories: string } }) {
  ...
  if (params.subCategories === 'all') {
    const { data } = await supabase.from('products').select('*, wishlist(*)').contains('sports', ['mma'])
    products = data as Array<Database['public']['Tables']['products']['Row']>
  } else if (params.subCategories === 'others') {
    const { data } = await supabase.from('products').select().contains('sports', ['mma']).or('equipment_type.in.(cup,bag,mouthguard,jumprope,gadget,headgear)')
    products = data as Array<Database['public']['Tables']['products']['Row']>
  } else {
    const { data } = await supabase.from('products').select().contains('sports', ['mma']).eq('equipment_type', params.subCategories)
    products = data as Array<Database['public']['Tables']['products']['Row']>
  }
