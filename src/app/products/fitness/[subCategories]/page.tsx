...
export default async function FitnessPage({ params }: { params: { subCategories: string } }) {
  ...
  if (params.subCategories === 'all') {
    const { data } = await supabase.from('products').select('*, wishlist(*)').contains('sports', ['fitness'])
    products = data as Array<Database['public']['Tables']['products']['Row']>
  } else if (params.subCategories === 'others') {
    const { data } = await supabase.from('products').select().contains('sports', ['fitness']).or('equipment_type.in.(jumprope,gadget)')
    products = data as Array<Database['public']['Tables']['products']['Row']>
  } else {
    const { data } = await supabase.from('products').select().contains('sports', ['fitness']).eq('equipment_type', params.subCategories)
    products = data as Array<Database['public']['Tables']['products']['Row']>
  }
