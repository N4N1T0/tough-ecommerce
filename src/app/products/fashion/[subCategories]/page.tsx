...
export default async function FashionPage({ params }: { params: { subCategories: string } }) {
  // Supabse CLient
  ...
  if (params.subCategories === 'all') {
    const { data } = await supabase.from('products').select('*, reviews(*)').contains('sports', ['fashion'])
    products = data as productsPropsWitReviews
  } else {
    const { data } = await supabase.from('products').select('*, reviews(*)').contains('sports', ['fashion']).eq('equipment_type', params.subCategories)
    products = data as productsPropsWitReviews
  }
    ...
    <ProductsLayout subCategory={params.subCategories} category='fashion'>
      <section>
        <div className='flex justify-between items-center'>
          ...
          <CategoriesMobileSheet category='fashion' subCategory={params.subCategories} />
        </div>
