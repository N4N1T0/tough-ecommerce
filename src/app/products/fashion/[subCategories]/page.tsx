...
export default async function FashionPage({ params }: { params: { subCategories: string } }) {
  ...
  if (params.subCategories === 'all') {
    const { data } = await supabase.from('products').select('*, wishlist(*)').contains('sports', ['fashion'])
    products = data as Array<Database['public']['Tables']['products']['Row']>
  } else {
    const { data } = await supabase.from('products').select().contains('sports', ['fashion']).eq('equipment_type', params.subCategories)
    products = data as Array<Database['public']['Tables']['products']['Row']>
  }
