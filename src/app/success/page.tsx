...
export default function SuccesPage() {
  ...
  return (
    <main className='max-w-[1000px] mx-auto px-5 flex flex-col justify-center items-center text-center gap-5 leading-10'>
      <header className='pt-48 py-10'>
        <h1 className='font-bold uppercase text-4xl xl:text-7xl'>Success Payment</h1>
        <h2 className='font-bold'>Your Order id is:  <span className='italic font-normal underline'>{sessionId}</span></h2>
      </header>
        ...
        {cartList?.map((item) => (
          <div key={item.id} className='flex gap-3 leading-5'>
            ...
            <div className='space-y-2'>
              ...
              <small className='block'>{item.description.split(' ').slice(0, 30).join(' ')}</small>
              <hr />
            </div>
          </div>
        ))}
